              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="flex gap-4">
                  <Avatar className="size-12">
                    {u.image && <AvatarImage src={u.image} alt={u.name || "User avatar"} />}
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="font-semibold text-[#101010]">{u.name || "Anonymous User"}</p>
                      <Badge variant="outline" className="rounded-full text-[#6b6b6b] border-[#e8e8e8]">
                        {(ROLE_LABELS && ROLE_LABELS[u.role]) || u.role || "User"}
                      </Badge>
                    </div>
                    <p className="text-sm text-[#6b6b6b]">{u.email || "No email"}</p>
                    {u.headline && (
                ...
                <span className="text-xs text-[#6b6b6b] whitespace-nowrap">
                  joined {u.createdAt ? timeAgo(u.createdAt) : "recently"}
                </span>
              </div>
